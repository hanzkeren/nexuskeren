...
use crate::runtime::start_parallel_authenticated_workers;
...
pub async fn setup_session(
    ...
    // Use all available CPU cores by default, respecting max_threads if specified
    let num_workers = if let Some(max) = max_threads {
        max as usize
    } else {
        // Let ParallelRunner determine optimal worker count based on system resources
    ...
    // Start parallel authenticated workers to utilize all CPU cores
    let (event_receiver, join_handles, max_tasks_shutdown_sender) = start_parallel_authenticated_workers(
        node_id,
        signing_key,
